...
#include <hbrs/mpl/dt/matrix_size/impl.hpp>
#include <memory>
...
py::module &
pydef_impl<hbrs::mpl::matrix_size_tag>::apply(py::module & m, py::module & base) {
	using hbrs::mpl::matrix_size;
	using hbrs::mpl::matrix_size_tag;
	auto py_matrix_size = py::class_<matrix_size_tag>{m, pystrip("matrix_size").c_str()};
	
	hana::for_each(
		hana::make_tuple(
			/* Both C++ types int and std::size_t will be mapped to Python's int type,
			 * hence dt.MatrixSize.make() will have multiple overloads:
			 *  1. make(arg0: int, arg1: int) -> edamer.cpp.dt.MatrixSize_Int_Int
			 *  2. make(arg0: int, arg1: int) -> edamer.cpp.dt.MatrixSize_StdSizeT_StdSizeT
			 * We list int before std::size_t to prioritize the MatrixSize_Int_Int overload.
			 */
			EDAMER_TYPE_NAME_PAIRS(int, int), /* El::Int is a typedef for int */
			EDAMER_TYPE_NAME_PAIRS(std::size_t, std::size_t)
		),
		[&m, &py_matrix_size](auto pairs) {
			auto types = hana::transform(pairs, hana::first);
			auto names = hana::transform(pairs, hana::second);
			auto name = boost::format("matrix_size<%s>") %
			...
			if constexpr (std::is_assignable_v<decltype(std::declval<type_t&>().m()), m_t&>) {
				c.def_property("m",
					[](type_t & sz) { return sz.m(); },
					[](type_t & sz, m_t & m) { sz.m() = HBRS_MPL_FWD(m); }
				);
			} else {
			...
			if constexpr (std::is_assignable_v<decltype(std::declval<type_t&>().n()), n_t&>) {
				c.def_property("n",
					[](type_t & sz) { return sz.n(); },
					[](type_t & sz, n_t & n) { sz.n() = HBRS_MPL_FWD(n); }
				);
			} else {
			...
			py_matrix_size.def_static("make",
